...
const Navbar = () => {
  ...
  const handleListProperty = async (e) => {
    ...
    try {
      ...
      const newcompany = await request("/company", 'POST', options, { ...state, img: filename })

      setShowModal(false)
      setShowForm(false)
      navigate(`/companyDetail/${newcompany._id}`)
    } catch (error) {
      setError(true)
        setTimeout(() => {
          setError(false)
            ...
            <>
              ...
              {showModal && (
                <div className={classes.userModal}>
                  ...
                  <Link onClick={() => setShowForm(true)} className={classes.list}>List your restaurant</Link>
                </div>
              )}
            </>
          }
        ...
        <div className={classes.listPropertyForm} onClick={handleCloseForm}>
          <div className={classes.listPropertyWrapper} onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleListProperty}>
              ...
              <input value={state?.rating} type="number" placeholder='rating' name="rating" onChange={handleState} />
              <input value={state?.review} type="text" placeholder='review' name="review" step={1} min={1} onChange={handleState} />
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', width: '50%' }}>
                <label htmlFor='photo'>Restaurant picture <AiOutlineFileImage /></label>
                <input
                  type="file"
                  id='photo'
                  style={{ display: 'none' }}
